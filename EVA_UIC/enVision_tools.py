...
class GenPattern:
    def __init__(self,PatName, PatType, Vector_List):
...
where: 
    ...
    def generate_signal_header(self,pin_list,SH_Name,isScanSH=False,scan_pin_req=None):
...
class GenPatternMap:
	...
	if self.D_pat_group != None:
	    self.PM_list.append('    DefaultGroup = "%s";'% (self.D_pat_group))
	self.PM_list.append('}')
